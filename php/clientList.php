...
switch ($level)  {
    case 'Все заказы':
        ...
        break;
    case 'Регионы':
        $query="Select DISTINCT ID,NAME from medline39.dbo.REGIONS where PARENT_ID=4";
        break;
    case 'Апт. Сети':
    ...
    case 'Аптеки':
        $query="Select F.ID,F.NAME,FTT.TYPE
                ...
                where F.DELETED<>100 and F.NODES_ID<>0 and F.PARENT_ID<>0 and FTT.TYPE=7";
        break;
    case 'Поставщики':
        $query="Select F.ID,F.NAME,FTT.TYPE
                from medline39.dbo.FIRMS as F
                inner join medline39.dbo.FIRM_TO_TYPES as FTT on F.ID=FTT.FIRMS_ID
                where F.DELETED<>100 and F.NODES_ID<>0 and F.PARENT_ID<>0 and FTT.TYPE=6";
        break;
    default:
